...
training_tile <- function(site="OSBS", year="2019", check.size=TRUE, savepath=NA,n=2) {
...
screenurls<-function(site,file.urls.current,savepath=savepath, n=2){
  ...
  file.urls.current <- file.urls.current %>% filter(stringr::str_detect(name,paste(selected_index, sep="", collapse = "|")))  %>% filter(stringr::str_detect(name,".laz|.h5|.tif"))
